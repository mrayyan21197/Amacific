...
export default function HomeMarketingCarousel() {
  ...
  const mutedTone = (theme) => {
    if (theme === "light") return "text-slate-700";
    if (theme === "mist") return "text-slate-700";
  ...
  return (
    <section className="relative home-campaign-carousel bg-white" aria-label="Featured campaigns">
      <Slider ref={sliderRef} {...settings}>
        {slides.map((s) => (
          <div key={s.id}>
            <div className="relative min-h-[390px] overflow-hidden sm:min-h-[420px] md:min-h-[480px] lg:min-h-[520px]">
              <img
                src={s.image}
                alt=""
              ...
              <div className="relative z-10 mx-auto flex h-full min-h-[390px] max-w-container items-center px-4 sm:min-h-[420px] md:min-h-[480px] md:px-8 lg:min-h-[520px]">
                <motion.div
                  initial={{ opacity: 0, y: 24 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.55 }}
                  className="max-w-xl py-10 sm:py-12 md:max-w-2xl"
                >
                  <p
                  ...
                  <h2 className={`mb-4 text-2xl font-titleFont font-extrabold leading-tight sm:text-3xl md:text-5xl ${textTone(s.theme)}`}>
                    {s.title}
                  </h2>
                  <p className={`mb-6 text-sm leading-relaxed sm:text-base md:text-lg ${mutedTone(s.theme)}`}>{s.subtitle}</p>
                  {s.code && (
                    <div
